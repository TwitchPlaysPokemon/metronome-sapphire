...
static const char * default_name = "Pokemon Ruby (English)";
static const char * default_code = "AXVE";
...
#define sym_get(name) (GetSymbolByName((name))->st_value)
    ...
    Elf32_Sym * TMHMMoves = GetSymbolByName("TMHMMoves");
    config_set("TMData", TMHMMoves->st_value);
    config_set("TotalTMsPlusHMs", TMHMMoves->st_size / 2);
    config_set("TotalTMs", TMHMMoves->st_size / 2 - 8);
    ...
    while (1) {
        ...
        Elf32_Sym * grp = GetSymbolByName(buffer);
        if (grp == NULL) break;
    ...
    Elf32_Sym * StarterMons = GetSymbolByName("sStarterMons");
    if (StarterMons != NULL) {
